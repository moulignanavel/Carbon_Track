...
export default function LeaderboardPositionCard({ user, allUsers = [], onLogActivity }) {
  ...
  return (
    <div className="relative overflow-hidden rounded-3xl bg-emerald-50/90 dark:bg-emerald-950/40 text-slate-900 dark:text-slate-50 p-6 shadow-sm border border-emerald-200/80 dark:border-emerald-800/60">
      ...
      <div className="relative z-10 flex flex-col lg:flex-row items-stretch lg:items-center justify-between gap-6">
        ...
        <div className="flex items-center gap-4">
          <div className="relative">
            <div className="w-16 h-16 rounded-2xl bg-emerald-100 dark:bg-emerald-900/60 border border-emerald-300 dark:border-emerald-700 flex items-center justify-center font-extrabold text-2xl text-emerald-800 dark:text-emerald-200 shadow-inner">
              {user.username ? user.username.charAt(0).toUpperCase() : 'Y'}
            </div>
            <span className="absolute -bottom-1.5 -right-1.5 px-2 py-0.5 rounded-full text-xs font-black bg-emerald-600 text-white shadow-md">
              #{userRank}
            </span>
          </div>
          ...
          <div>
            <div className="flex items-center gap-2 mb-1">
              <span className="px-2.5 py-0.5 rounded-full text-[11px] font-bold bg-emerald-100 dark:bg-emerald-900/50 text-emerald-800 dark:text-emerald-300 border border-emerald-200 dark:border-emerald-700/60">
                Your Position
              </span>
              <span className="text-xs text-slate-500 dark:text-slate-400 font-semibold">Global Rank #{userRank}</span>
            </div>
            <h3 className="text-xl font-black tracking-tight text-slate-900 dark:text-slate-50">
              {user.username}
            </h3>
            <p className="text-xs text-slate-600 dark:text-slate-300 flex items-center gap-2 mt-1 font-medium">
        ...
        <div className="flex-1 max-w-md bg-white/90 dark:bg-slate-900/90 border border-emerald-200/80 dark:border-emerald-800/60 rounded-2xl p-4 shadow-sm">
          <div className="flex items-center justify-between text-xs font-semibold mb-2">
            <span className="text-emerald-800 dark:text-emerald-300 flex items-center gap-1.5 font-bold">
          ...
          <div className="w-full h-3 rounded-full bg-slate-100 dark:bg-slate-800 border border-slate-200 dark:border-slate-700 overflow-hidden mb-2">
            <div
        ...
        <div className="flex items-center gap-3">
          <Button
            ...
            className="
              px-5 py-3 rounded-xl font-bold text-sm text-white 
              bg-emerald-600 hover:bg-emerald-700
              shadow-md shadow-emerald-600/20 transition-all transform hover:scale-105
              flex items-center gap-2 whitespace-nowrap border-0
            "
          >
