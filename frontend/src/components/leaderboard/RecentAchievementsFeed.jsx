...
export default function RecentAchievementsFeed({ achievements = [] }) {
  ...
  return (
    <div className="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-3xl p-5 shadow-sm">
      ...
      {feed.length === 0 ? (
        <div className="p-6 text-center rounded-2xl bg-slate-50 dark:bg-slate-800/40 border border-dashed border-slate-200 dark:border-slate-800">
        ...
        <div className="space-y-3">
          {feed.map((item, idx) => {
            ...
            return (
              <div
                key={item.id || idx}
                className="flex items-center gap-3 p-3 rounded-2xl bg-slate-50/60 dark:bg-slate-800/40 border border-slate-100 dark:border-slate-800 hover:bg-slate-100/60 dark:hover:bg-slate-800/80 transition-colors"
              >
