...
export default function ProfileSettings({ onProfileUpdate }) {
  ...
  const handleUpdate = async (e) => {
    ...
    try {
      const response = await api.updateProfile(username, email, preferredUnit, goalVisibility);
    ...
    <div className="max-w-2xl mx-auto p-4 animate-fade-in">
      <div className="glass-panel p-8">
        <div className="flex items-center gap-3 mb-6">
          ...
          <div>
            <h2 className="text-2xl font-bold">Profile & Preferences</h2>
            <p className="text-sm text-gray-400">Manage settings for unit measurements and visibility</p>
          </div>
        </div>
        ...
        <form onSubmit={handleUpdate} className="space-y-6">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6 pt-4 border-t border-white/5">
            <div>
              <label className="block text-xs font-semibold text-gray-400 mb-2 uppercase tracking-wider">Preferred Unit System</label>
              <div className="relative">
                <span className="absolute inset-y-0 left-0 pl-3.5 flex items-center pointer-events-none">
                ...
                >
                  <option value="km" className="bg-slate-900">Metric (km / kWh / servings)</option>
                  <option value="miles" className="bg-slate-900">Imperial (miles / kWh / servings)</option>
                </select>
              </div>
            </div>

            <div>
              <label className="block text-xs font-semibold text-gray-400 mb-2 uppercase tracking-wider">Goal Visibility</label>
              <div className="relative">
                <span className="absolute inset-y-0 left-0 pl-3.5 flex items-center pointer-events-none">
                ...
                >
                  <option value="public" className="bg-slate-900">Public (Visible to Organization)</option>
                  <option value="private" className="bg-slate-900">Private (Only me)</option>
                </select>
              </div>
            </div>
          </div>
