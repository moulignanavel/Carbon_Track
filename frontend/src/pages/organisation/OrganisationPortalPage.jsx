...
function DashboardHero({data}){
  ...
  const metrics=[
    ['Sustainability score',hasScore?<><AnimatedValue value={kpis.averageCarbonScore}/><span className="text-xs font-medium text-emerald-200"> / 100</span></>:'N/A'],
    ['Reporting period',latestPeriod||'N/A'],
    ['Monthly carbon reduction',hasReduction?<><AnimatedValue value={kpis.monthlyReduction} suffix="%"/></>:'N/A'],
    ['Participation rate',hasParticipation?<><AnimatedValue value={kpis.participationRate} suffix="%"/></>:'N/A'],
  ];
  return <motion.section initial={reduceMotion?false:{opacity:0,y:10}} animate={{opacity:1,y:0}} className="relative overflow-hidden rounded-2xl border border-emerald-800/60 px-4 py-3.5 text-white shadow-md sm:px-5 sm:py-4">
    <EcoBackground/>
    <div className="relative z-10 grid gap-4 lg:grid-cols-[minmax(0,1fr)_140px] lg:items-center">
      <div className="min-w-0">
        <div className="inline-flex max-w-full items-center gap-1.5 rounded-full border border-white/15 bg-white/[.08] px-2.5 py-0.5 text-[11px] font-semibold text-emerald-100 backdrop-blur-sm">
          ...
          <span className="truncate">{data.organisation?.name||'Organisation'}</span>
        </div>
        <h1 className="mt-2 text-lg font-black tracking-tight sm:text-xl leading-tight">Organisation Sustainability Overview</h1>
        <p className="mt-1 text-xs text-emerald-50/80 leading-relaxed">Scope emissions, employee participation, and CSR sustainability benchmarks.</p>
        ...
        <div className="mt-3">
          <Link to="/organisation/reports" className="group inline-flex min-h-9 items-center gap-2 rounded-lg bg-white px-3 py-1.5 text-emerald-950 shadow-sm transition hover:bg-emerald-50">
            ...
            <span className="text-xs font-bold">View CSR Reports →</span>
          </Link>
        </div>
      </div>
      <div className="mx-auto w-full max-w-[130px] rounded-xl border border-white/15 bg-white/[.07] p-2 backdrop-blur-sm lg:mx-0">
        <EcoLottie animationData={earthAnimation} loop className="aspect-square w-full" fallback={<EcoStaticFallback type="earth"/>} reducedMotionFallback={<EcoStaticFallback type="earth"/>}/>
      </div>
    </div>
  </motion.section>;
}
...
function KpiCard({item,index,reduceMotion}){
  ...
  return <motion.article initial={reduceMotion?false:{opacity:0,y:8}} animate={{opacity:1,y:0}} transition={{delay:index*.04,duration:.25}} whileHover={reduceMotion?undefined:{y:-2}} className="group flex flex-col justify-between min-h-[92px] rounded-xl border border-slate-200/80 bg-white p-3 shadow-xs transition-all hover:border-emerald-300 dark:border-slate-800 dark:bg-slate-900">
    <div className="flex items-start justify-between gap-2">
      <div className="min-w-0">
        ...
        {hasValue ? (
          <p className="mt-1 flex items-baseline gap-1 text-lg font-black tracking-tight text-slate-900 dark:text-white">
          ...
          <p className="mt-1 text-xs font-semibold text-slate-400">No data</p>
        )}
      </div>
...
function OrganisationKpiCards({data,loading,error,onRetry}){
  ...
  const changeLabel=(amount,invert=false)=>amount===null?null:`${Math.abs(amount).toFixed(1)}% ${amount===0?'same':invert?(amount<0?'lower':'higher'):(amount>0?'higher':'lower')}`;
  const trendFor=(amount,invert=false)=>amount===null||amount===0?'neutral':(invert?amount<0:amount>0)?'positive':'negative';
  const items=[
    {label:'Total Emissions',value:kpis.totalEmission,unit:'kg CO₂e',Icon:Leaf,change:changeLabel(emissionDelta,true),trend:trendFor(emissionDelta,true),sparkline:monthly,positiveWhenLower:true},
    {label:'Carbon Reduction',value:reduction,unit:'%',Icon:TrendingDown,change:reduction===null?null:`${Math.abs(reduction).toFixed(1)}% ${reduction===0?'same':reduction>0?'reduced':'increased'}`,trend:trendFor(reduction),positiveWhenLower:false},
    {label:'Active Employees',value:activeEmployees,unit:'active',Icon:Users,trend:'neutral'},
    {label:'Participation Rate',value:kpis.participationRate,unit:'%',Icon:Activity,trend:'neutral'},
    {label:'Active Goals',value:kpis.activeGoals,unit:'goals',Icon:Target,trend:'neutral'},
    {label:'Sustainability Score',value:kpis.averageCarbonScore,unit:'/ 100',Icon:Star,trend:'neutral'},
  ];
...
function CarbonEmissionsAnalyticsSection({data,loading,error,onRetry}){
  ...
  const monthly=(data.monthlyEmissions||[]).slice(-6);
  ...
  return (
    <div className="space-y-2.5">
      <ExecutiveSectionHeader 
        ...
        subtitle="Verified emissions breakdown & monthly trends" 
        badge="Scope Analytics"
      />
      <div className="grid gap-3.5 xl:grid-cols-2">
        <ChartCard title="Monthly Emission Trend">
          {monthly.length ? (
            <ResponsiveContainer>
              <AreaChart data={monthly} margin={{top:4,right:8,bottom:12,left:2}}>
                ...
                <Area name="Emissions" type="monotone" dataKey="emissions" stroke="#059669" strokeWidth={2} fill="url(#dashboardEmissionFill)" isAnimationActive={animate}/>
              </AreaChart>
            </ResponsiveContainer>
          ) : <Empty label="monthly emission data"/>}
        </ChartCard>
        ...
        <ChartCard title="Emissions by Category">
          {categories.length ? (
            <ResponsiveContainer>
              <PieChart>
                <Pie data={categories} dataKey="emissions" nameKey="category" innerRadius="50%" outerRadius="75%" paddingAngle={2} isAnimationActive={animate}>
          ...
          ) : <Empty label="category emission data"/>}
        </ChartCard>
      </div>
    </div>
  );
...
function DepartmentComparisonSection({data,loading}){
  ...
  const animate=!reduceMotion&&!animatedRef.current;
  useEffect(()=>{animatedRef.current=true},[]);
  const departments=[...(data.departmentComparison||[])].sort((a,b)=>Number(a.emissions)-Number(b.emissions));
  ...
  return (
    <div className="space-y-2.5">
      <ExecutiveSectionHeader 
        ...
        subtitle="Departmental carbon footprints ranked from lowest to highest" 
        badge="Operational Ranking"
        action={
      ...
      />
      <div className="grid gap-3.5 xl:grid-cols-2">
        <ChartCard title="Department Footprint Chart">
          {departments.length ? (
            <ResponsiveContainer>
              <BarChart data={departments} layout="vertical" margin={{top:2,right:12,bottom:12,left:4}}>
                <CartesianGrid stroke="#e2e8f0" strokeDasharray="3 3" horizontal={false}/>
                <XAxis type="number" tickLine={false} axisLine={false} tick={{fontSize:10,fill:'#64748b'}}/>
                <YAxis type="category" dataKey="department" width={90} tickLine={false} axisLine={false} tick={{fontSize:10,fill:'#64748b'}}/>
                <Tooltip content={<DashboardChartTooltip/>}/>
                <Bar name="Emissions" dataKey="emissions" fill="#0f766e" radius={[0,4,4,0]} isAnimationActive={animate}/>
              </BarChart>
            </ResponsiveContainer>
          ) : <Empty label="department performance data"/>}
        </ChartCard>

        <section className={card}>
          <DashboardSectionHeader title="Department Performance Insights" description="Summary takeaways from current department data."/>
          {departments.length ? (
            <div className="space-y-2">
              <div className="rounded-lg border border-emerald-100 bg-emerald-50/60 p-2.5 dark:border-emerald-900/60 dark:bg-emerald-950/30">
                <p className="text-[10px] font-bold uppercase tracking-wider text-emerald-800 dark:text-emerald-300">Leading Department</p>
                <p className="mt-0.5 text-sm font-extrabold text-slate-900 dark:text-white">{bestDepartment?.department || '—'}</p>
                <p className="text-[11px] text-slate-500">Lowest footprint: {bestDepartment ? `${Number(bestDepartment.emissions).toLocaleString()} kg CO₂e` : '—'}</p>
              </div>
              <div className="grid grid-cols-2 gap-2">
                <div className="rounded-lg border border-slate-100 bg-slate-50/70 p-2.5 dark:border-slate-800 dark:bg-slate-950/30">
                  <p className="text-[10px] font-medium text-slate-500">Total Depts</p>
                  <p className="mt-0.5 text-sm font-black text-slate-900 dark:text-white">{departments.length}</p>
                </div>
                <div className="rounded-lg border border-slate-100 bg-slate-50/70 p-2.5 dark:border-slate-800 dark:bg-slate-950/30">
                  <p className="text-[10px] font-medium text-slate-500">Avg per Dept</p>
          ...
          ) : <Empty label="department insights"/>}
        </section>
      </div>
    </div>
  );
...
function EmployeePerformanceSection({data,loading}){
  ...
  const participation=useMemo(()=>{
    ...
    for(let offset=5;offset>=0;offset--){const date=new Date(now.getFullYear(),now.getMonth()-offset,1);const year=date.getFullYear(),month=date.getMonth();const participants=new Set(logs.filter(log=>{const logged=new Date(`${log.date}T00:00:00`);return !Number.isNaN(logged.getTime())&&logged.getFullYear()===year&&logged.getMonth()===month}).map(log=>log.employee).filter(Boolean));points.push({month:date.toLocaleString('en',{month:'short'}),rate:Number((participants.size*100/employees.length).toFixed(1))})}
    return points;
  },[data.activityLogs,data.employees]);
  ...
  return (
    <div className="space-y-2.5">
      <ExecutiveSectionHeader 
        title="Employee Performance & Participation" 
        subtitle="Workforce engagement, top carbon savers, and recent activities" 
        badge="Workforce ESG"
        action={
          ...
          {participation.length ? (
            <ResponsiveContainer>
              <LineChart data={participation} margin={{top:4,right:8,bottom:12,left:2}}>
                ...
                <Tooltip content={<DashboardChartTooltip/>}/>
                <Line name="Participation" type="monotone" dataKey="rate" stroke="#3b82f6" strokeWidth={2} dot={{r:2.5,fill:'#3b82f6'}} activeDot={{r:4}} isAnimationActive={animate}/>
              </LineChart>
            </ResponsiveContainer>
          ) : <Empty label="participation history"/>}
        </ChartCard>

        <section className={card}>
          <DashboardSectionHeader title="Top Sustainability Champions" description="Ranked by verified carbon savings." to="/organisation/top-contributors" linkLabel="Rankings"/>
          {contributors.length ? (
              ...
              {contributors.map((employee,index)=>(
                <li key={employee.id||index} className="flex items-center gap-2 rounded-lg border border-slate-100 p-2 dark:border-slate-800/80">
                  ...
                  <div className="min-w-0 flex-1">
                    <p className="truncate text-xs font-bold text-slate-900 dark:text-white">{employee.name||'Employee'}</p>
                    <p className="truncate text-[10px] text-slate-400">{employee.department||'General'}</p>
                  </div>
          ...
          ) : <Empty label="contributor data"/>}
        </section>
      </div>

      <section className={card}>
        <DashboardSectionHeader title="Recent Organisation Activities" description="Latest verified activities submitted by employees." to="/organisation/activity-logs" linkLabel="View logs"/>
        {recent.length ? (
            ...
            <table className="w-full min-w-[600px] text-left text-xs">
              <thead>
                <tr className="border-b border-slate-200 text-[10px] uppercase tracking-wider text-slate-500 dark:border-slate-700">
                  {['Employee','Category','Emission','Date','Status'].map(label=><th key={label} className="px-2.5 py-1.5 font-semibold">{label}</th>)}
                </tr>
              </thead>
              <tbody>
                {recent.map(activity=>(
                  <tr key={activity.id} className="border-b border-slate-100 last:border-0 dark:border-slate-800/60 hover:bg-slate-50/50 dark:hover:bg-slate-800/40">
                    <td className="px-2.5 py-1.5 font-bold text-slate-900 dark:text-white">{activity.employee||'Employee'}</td>
                    <td className="px-2.5 py-1.5"><span className="rounded-full bg-emerald-50 px-2 py-0.5 text-[10px] font-semibold text-emerald-700 dark:bg-emerald-950/40 dark:text-emerald-300">{activity.category||'General'}</span></td>
                    ...
                    <td className="px-2.5 py-1.5"><span className="rounded-full bg-blue-50 px-1.5 py-0.5 text-[9px] font-bold text-blue-700 dark:bg-blue-950/40 dark:text-blue-300">{activity.status||'VERIFIED'}</span></td>
                  </tr>
                ))}
              </tbody>
            </table>
          </div>
        ) : <Empty label="recent organisation activities"/>}
      </section>
    </div>
  );
}
...
function MonthlyTrendsAndGoalsSection({data,loading}){
  ...
  return (
    <div className="space-y-2.5">
      <ExecutiveSectionHeader 
        title="Goals & Reports" 
        subtitle="Active ESG targets and compliance reports" 
        badge="CSR Targets"
        action={
      ...
      <div className="grid gap-3.5 xl:grid-cols-2">
        <section className={card}>
          <DashboardSectionHeader title="Active Sustainability Goals" description="Corporate emissions reduction targets." to="/organisation/goals" linkLabel="All goals"/>
          {goals.length ? (
            <div className="space-y-2">
              {goals.map(goal=>{
                ...
                return (
                  <div key={goal.id} className="rounded-lg border border-slate-100 p-2.5 dark:border-slate-800">
                    <div className="flex items-start justify-between gap-2">
                      <div className="min-w-0">
                        <p className="truncate text-xs font-bold text-slate-900 dark:text-white">{goal.title}</p>
                        <p className="mt-0.5 text-[10px] text-slate-500">Target: {Number.isFinite(target)?`${target.toLocaleString()} kg`:'—'} · Deadline: {goal.endDate||'Not set'}</p>
                      </div>
                      <span className="shrink-0 rounded-full bg-emerald-50 px-2 py-0.5 text-[10px] font-bold text-emerald-700 dark:bg-emerald-950/40 dark:text-emerald-300">{goal.status}</span>
                    </div>
          ...
          ) : <Empty label="active organisation goals"/>}
        </section>
        ...
        <section className="relative overflow-hidden rounded-xl border border-emerald-800/40 bg-gradient-to-br from-emerald-950 via-slate-900 to-emerald-900 p-4 text-white shadow-sm flex flex-col justify-between">
          <div>
            ...
            <h3 className="text-sm font-extrabold text-white">CSR & Sustainability Reports</h3>
            <p className="mt-1 text-[11px] leading-relaxed text-slate-300">
              Export executive PDF or CSV sustainability reports covering Scope 1, 2 & 3 emissions.
            </p>
          </div>
          <div className="mt-3 flex flex-wrap gap-2">
            <Link to="/organisation/reports" className="inline-flex items-center justify-center gap-1.5 rounded-lg bg-emerald-500 px-3 py-1.5 text-xs font-bold text-slate-950 hover:bg-emerald-400 transition-all">
              <Eye className="h-3.5 w-3.5"/> Generate CSR Report
            </Link>
          </div>
        </section>
      </div>
...
function DashboardCharts({data,loading,error,onRetry}){
  const [range,setRange]=useState(6);const reduceMotion=useReducedMotion();const animatedRef=useRef(false);const animate=!reduceMotion&&!animatedRef.current;
  useEffect(()=>{animatedRef.current=true},[]);
  const monthly=(data.monthlyEmissions||[]).slice(-range);
  const categories=data.categoryBreakdown||[];const departments=[...(data.departmentComparison||[])].sort((a,b)=>Number(a.emissions)-Number(b.emissions));
  const participation=useMemo(()=>{
    const employees=data.employees||[];if(!employees.length)return[];
    const logs=data.activityLogs||[];const now=new Date();const points=[];
    ...
    return points;
  },[data.activityLogs,data.employees,range]);
  ...
  return <section aria-labelledby="dashboard-chart-heading" className="space-y-4">
    <div className="flex flex-col gap-3 sm:flex-row sm:items-end sm:justify-between"><div><h2 id="dashboard-chart-heading" className="text-lg font-semibold text-slate-950 dark:text-white">Sustainability performance</h2><p className="mt-1 text-sm text-slate-500">Charts use verified organisation activity and goal data.</p></div><label className="w-full sm:w-auto"><span className="mb-1 block text-xs font-medium text-slate-500">Dated trend range</span><select className={`${input} sm:w-44`} value={range} onChange={event=>setRange(Number(event.target.value))}><option value={6}>Last 6 months</option><option value={12}>Last 12 months</option></select></label></div>
    <div className="grid gap-5 xl:grid-cols-2">
      <DashboardChartCard title="Monthly Emission Trend" description={`Last ${range} months · verified emissions`} summary={latest?`Latest monthly emissions are ${latest.emissions} kilograms of carbon dioxide equivalent in ${latest.month}.`:'No monthly emission data is available.'}>
        {monthly.length?<div className="h-72"><ResponsiveContainer><AreaChart data={monthly} margin={{top:8,right:12,bottom:18,left:6}}><defs><linearGradient id="dashboardEmissionFill" x1="0" y1="0" x2="0" y2="1"><stop offset="0" stopColor="#059669" stopOpacity=".3"/><stop offset="1" stopColor="#059669" stopOpacity=".02"/></linearGradient></defs><CartesianGrid stroke="#e2e8f0" strokeDasharray="3 3" vertical={false}/><XAxis dataKey="month" tickLine={false} axisLine={false} tick={{fontSize:11,fill:'#64748b'}} label={{value:'Reporting month',position:'insideBottom',offset:-12,fill:'#64748b',fontSize:11}}/><YAxis tickLine={false} axisLine={false} width={52} tick={{fontSize:11,fill:'#64748b'}} label={{value:'kg CO₂e',angle:-90,position:'insideLeft',fill:'#64748b',fontSize:11}}/><Tooltip content={<DashboardChartTooltip/>}/><Area name="Emissions" type="monotone" dataKey="emissions" stroke="#059669" strokeWidth={2.5} fill="url(#dashboardEmissionFill)" isAnimationActive={animate}/></AreaChart></ResponsiveContainer></div>:<Empty label="monthly emission data"/>}
      </DashboardChartCard>
      <DashboardChartCard title="Emissions by Category" description="All verified activity currently available" summary={highestCategory?`${highestCategory.category} is the largest category at ${highestCategory.emissions} kilograms of carbon dioxide equivalent.`:'No category emission data is available.'}>
        {categories.length?<div className="h-72"><ResponsiveContainer><PieChart><Pie data={categories} dataKey="emissions" nameKey="category" innerRadius="52%" outerRadius="78%" paddingAngle={2} isAnimationActive={animate}>{categories.map((row,index)=><Cell key={row.category} fill={palette[index%palette.length]}/>)}</Pie><Tooltip content={<DashboardChartTooltip/>}/><Legend iconType="circle" wrapperStyle={{fontSize:'11px'}}/></PieChart></ResponsiveContainer></div>:<Empty label="category emission data"/>}
      </DashboardChartCard>
      <DashboardChartCard title="Department Performance" description="Lower emissions indicate stronger performance" summary={bestDepartment?`${bestDepartment.department} currently has the lowest recorded emissions at ${bestDepartment.emissions} kilograms of carbon dioxide equivalent.`:'No department performance data is available.'}>
        {departments.length?<div className="h-72"><ResponsiveContainer><BarChart data={departments} layout="vertical" margin={{top:4,right:18,bottom:20,left:8}}><CartesianGrid stroke="#e2e8f0" strokeDasharray="3 3" horizontal={false}/><XAxis type="number" tickLine={false} axisLine={false} tick={{fontSize:11,fill:'#64748b'}} label={{value:'Emissions (kg CO₂e)',position:'insideBottom',offset:-12,fill:'#64748b',fontSize:11}}/><YAxis type="category" dataKey="department" width={100} tickLine={false} axisLine={false} tick={{fontSize:11,fill:'#64748b'}}/><Tooltip content={<DashboardChartTooltip/>}/><Bar name="Emissions" dataKey="emissions" fill="#0f766e" radius={[0,5,5,0]} isAnimationActive={animate}/></BarChart></ResponsiveContainer></div>:<Empty label="department performance data"/>}
      </DashboardChartCard>
      <DashboardChartCard title="Participation Trend" description={`Monthly activity participation · last ${range} months`} summary={latestParticipation?`The latest participation rate derived from recorded activities is ${latestParticipation.rate} percent.`:'No participation history is available.'}>
        {participation.length?<div className="h-72"><ResponsiveContainer><LineChart data={participation} margin={{top:8,right:12,bottom:18,left:6}}><CartesianGrid stroke="#e2e8f0" strokeDasharray="3 3" vertical={false}/><XAxis dataKey="month" tickLine={false} axisLine={false} tick={{fontSize:11,fill:'#64748b'}} label={{value:'Reporting month',position:'insideBottom',offset:-12,fill:'#64748b',fontSize:11}}/><YAxis domain={[0,100]} tickLine={false} axisLine={false} width={45} tick={{fontSize:11,fill:'#64748b'}} label={{value:'Rate %',angle:-90,position:'insideLeft',fill:'#64748b',fontSize:11}}/><Tooltip content={<DashboardChartTooltip/>}/><Line name="Participation" type="monotone" dataKey="rate" stroke="#3b82f6" strokeWidth={2.5} dot={{r:3,fill:'#3b82f6'}} activeDot={{r:5}} isAnimationActive={animate}/></LineChart></ResponsiveContainer></div>:<Empty label="participation history"/>}
      </DashboardChartCard>
      <DashboardChartCard title="Goal Progress" description="Progress from active organisation goals" summary={goals.length?`${goals.length} active goals are displayed with their current completion percentages.`:'No active goal data is available.'} className="xl:col-span-2">
        {goals.length?<div className="grid gap-4 md:grid-cols-2">{goals.map(goal=>{const target=Number(goal.targetKg);const current=Number(goal.currentKg);const progress=target>0?Math.min(100,Math.max(0,current*100/target)):0;return <div key={goal.id} className="rounded-xl border border-slate-100 bg-slate-50/60 p-4 dark:border-slate-800 dark:bg-slate-950/30"><div className="flex items-center justify-between gap-4"><p className="truncate text-sm font-semibold text-slate-800 dark:text-slate-100">{goal.title}</p><span className="text-sm font-bold text-emerald-700 dark:text-emerald-300">{progress.toFixed(0)}%</span></div><div className="mt-3 h-2.5 overflow-hidden rounded-full bg-slate-200 dark:bg-slate-700"><motion.div initial={animate?{width:0}:false} animate={{width:`${progress}%`}} transition={{duration:.65,ease:'easeOut'}} className="h-full rounded-full bg-emerald-600"/></div><p className="mt-2 text-xs text-slate-500">{Number.isFinite(current)?current.toLocaleString():0} of {Number.isFinite(target)?target.toLocaleString():0} kg CO₂e</p></div>})}</div>:<Empty label="active goal data"/>}
      </DashboardChartCard>
    </div>
  </section>;
}
function AnalyticsPage({data}){
  ...
  const analyticsData=useMemo(()=>{
    ...
    const aggregate=(key,getKey)=>[...chronological.reduce((map,row)=>{const name=getKey(row);if(name)map.set(name,(map.get(name)||0)+Number(row.emission||0));return map},new Map())].map(([name,emissions])=>({[key]:name,emissions:Number(emissions.toFixed(2))}));
  ...
  const insights=[['Highest-emission category',highestCategory?.category||'No category data',highestCategory?`${Number(highestCategory.emissions).toLocaleString()} kg CO₂e`:'No matching activity data'],['Best-performing department',bestDepartment?.department||'No department data',bestDepartment?`${Number(bestDepartment.emissions).toLocaleString()} kg CO₂e`:'No matching department data'],['Filtered activities',filtered.length,range==='all'?'All available verified records':'Verified records in selected period'],['Participation rate',analyticsData.kpis?.participationRate===null?'Not available':`${analyticsData.kpis?.participationRate}%`,`${analyticsData.kpis?.totalEmployees||0} eligible employees`]];
  return <div className="space-y-5">
    <section className={`${card} sticky top-[84px] z-10`}>
      <div className="grid gap-3 md:grid-cols-3">
        <label><span className="mb-1.5 block text-xs font-medium text-slate-500">Date range</span><select className={input} aria-label="Analytics date range" value={range} onChange={event=>setRange(event.target.value)}><option value="all">All available dates</option><option value="month">This month</option><option value="quarter">This quarter</option><option value="year">This year</option></select></label>
        <label><span className="mb-1.5 block text-xs font-medium text-slate-500">Department</span><select className={input} aria-label="Analytics department" value={department} onChange={event=>setDepartment(event.target.value)}><option value="">All departments</option>{departmentOptions.map(value=><option key={value} value={value}>{value}</option>)}</select></label>
        <label><span className="mb-1.5 block text-xs font-medium text-slate-500">Category</span><select className={input} aria-label="Analytics category" value={category} onChange={event=>setCategory(event.target.value)}><option value="">All categories</option>{categoryOptions.map(value=><option key={value} value={value}>{value}</option>)}</select></label>
      </div>
      <div className="mt-3 flex flex-wrap items-center justify-between gap-2 border-t border-slate-100 pt-3 text-xs text-slate-500 dark:border-slate-800"><span>Showing {filtered.length.toLocaleString()} matching verified activities</span>{(range!=='all'||department||category)&&<button type="button" onClick={()=>{setRange('all');setDepartment('');setCategory('')}} className="font-semibold text-emerald-700 hover:underline dark:text-emerald-300">Clear filters</button>}</div>
    </section>
    ...
    {!filtered.length&&<div className="rounded-2xl border border-dashed border-emerald-200 bg-emerald-50/50 px-5 py-6 text-center dark:border-emerald-900 dark:bg-emerald-950/20"><EcoLottie animationData={emptyAnimation} className="mx-auto h-28 w-28" fallback={<EcoStaticFallback/>} reducedMotionFallback={<EcoStaticFallback/>}/><h3 className="mt-2 font-semibold text-slate-900 dark:text-white">No analytics match these filters</h3><p className="mt-1 text-sm text-slate-500">Try another date range, department, or activity category.</p></div>}
    <TrendCharts data={analyticsData}/>
    <ChartCard title="Participation and Goal Completion"><ResponsiveContainer><BarChart data={[{name:'Participation',value:analyticsData.kpis.participationRate||0},{name:'Goal completion',value:data.goals.length?data.goals.filter(goal=>goal.status==='ACHIEVED').length*100/data.goals.length:0}]}><CartesianGrid strokeDasharray="3 3" opacity=".18"/><XAxis dataKey="name"/><YAxis domain={[0,100]}/><Tooltip formatter={value=>[`${Number(value).toFixed(1)}%`,'Rate']}/><Bar dataKey="value" fill="#059669" radius={[6,6,0,0]}/></BarChart></ResponsiveContainer></ChartCard>
  </div>;
}
function MonthlyTrendPage({data}){
  ...
  return <div className="space-y-5">
    <div className="grid gap-3 sm:grid-cols-3"><article className={card}><p className="text-xs text-slate-500">Latest month</p><p className="mt-2 text-xl font-bold">{latest?.month||'—'}</p></article><article className={card}><p className="text-xs text-slate-500">Latest emissions</p><p className="mt-2 text-xl font-bold">{latest?`${Number(latest.emissions||0).toLocaleString()} kg CO₂e`:'—'}</p></article><article className={card}><p className="text-xs text-slate-500">Month-over-month change</p><p className={`mt-2 text-xl font-bold ${change<=0?'text-emerald-600':'text-red-600'}`}>{previous?`${change>0?'+':''}${change.toFixed(1)}%`:'—'}</p></article></div>
    <ChartCard title="Monthly Emission Trend">
      {data.monthlyEmissions?.length?<ResponsiveContainer><AreaChart data={data.monthlyEmissions}><defs><linearGradient id="monthlyOnlyGreen" x1="0" y1="0" x2="0" y2="1"><stop offset="0" stopColor="#059669" stopOpacity=".4"/><stop offset="1" stopColor="#059669" stopOpacity=".03"/></linearGradient></defs><CartesianGrid strokeDasharray="3 3" opacity=".18"/><XAxis dataKey="month"/><YAxis/><Tooltip/><Area type="monotone" dataKey="emissions" stroke="#059669" strokeWidth={2} fill="url(#monthlyOnlyGreen)"/></AreaChart></ResponsiveContainer>:<Empty label="monthly emission data"/>}
    </ChartCard>
    <section className={card}>
      <h2 className="mb-4 text-base font-semibold">Monthly records</h2>
      <div className="overflow-x-auto"><table className="w-full min-w-[480px] text-sm"><thead className="bg-slate-50 text-left text-xs uppercase text-slate-500"><tr><th className="px-4 py-3">Month</th><th className="px-4 py-3">Emissions</th></tr></thead><tbody>{data.monthlyEmissions?.map(row=><tr key={row.month} className="border-t border-slate-100"><td className="px-4 py-3 font-medium">{row.month}</td><td className="px-4 py-3">{Number(row.emissions||0).toLocaleString()} kg CO₂e</td></tr>)}</tbody></table>{!data.monthlyEmissions?.length&&<Empty label="monthly records"/>}</div>
    </section>
  </div>;
}
function EmployeePage({rows,reload}){
  ...
  return <div className="space-y-4"><div className="grid gap-3 sm:grid-cols-3"><article className={card}><p className="text-xs text-slate-500">Total employees</p><p className="mt-2 text-2xl font-bold">{rows.length}</p></article><article className={card}><p className="text-xs text-slate-500">Active participants</p><p className="mt-2 text-2xl font-bold text-emerald-600">{active}</p></article><article className={card}><p className="text-xs text-slate-500">Departments · Average score</p><p className="mt-2 text-2xl font-bold">{departments} · {average.toFixed(1)}</p></article></div><AddEmployee reload={reload}/><Employees rows={rows} reload={reload}/></div>;
}
function ContributorPage({rows=[],lowest=false}){
  ...
  return <div className="space-y-4"><div className="grid gap-3 sm:grid-cols-3"><article className={card}><p className="text-xs text-slate-500">Ranked employees</p><p className="mt-2 text-2xl font-bold">{rows.length}</p></article><article className={card}><p className="text-xs text-slate-500">{lowest?'Average footprint':'Total carbon saved'}</p><p className="mt-2 text-2xl font-bold text-emerald-600">{lowest?`${average.toFixed(1)} kg CO₂e`:`${totalSaved.toLocaleString()} kg CO₂e`}</p></article><article className={card}><p className="text-xs text-slate-500">Ranking basis</p><p className="mt-2 text-base font-semibold">Verified activity data</p></article></div>{lowest&&<div className="rounded-xl border border-blue-200 bg-blue-50 px-4 py-3 text-sm text-blue-800 dark:border-blue-900 dark:bg-blue-950/40 dark:text-blue-200">Rankings depend on verified activities recorded during the available reporting period.</div>}<Ranking rows={rows} lowest={lowest}/></div>;
}

function DepartmentPage({rows=[]}){
  const sorted=[...rows].sort((a,b)=>Number(a.emissions||0)-Number(b.emissions||0));
  ...
  const metrics=[
    ['Departments',sorted.length,'Departments currently reporting'],
    ['Total Emissions',`${total.toLocaleString()} kg CO₂e`,'Across all departments'],
    ['Lowest Footprint',best?.department||'—',best?`${Number(best.emissions||0).toLocaleString()} kg CO₂e`:'No data available'],
    ['Average per Employee',employees?`${(total/employees).toFixed(1)} kg CO₂e`:'—',`${employees.toLocaleString()} participating employees`],
  ];
  return <div className="space-y-5">
    ...
    <section className={card}>
      <div className="mb-5"><h2 className="text-base font-semibold">Emission comparison</h2><p className="mt-1 text-sm text-slate-500">Lower emissions indicate stronger current performance.</p></div>
      <div className="h-[340px]">{sorted.length?<ResponsiveContainer><BarChart data={sorted} layout="vertical" margin={{left:16,right:28}}><CartesianGrid strokeDasharray="3 3" horizontal={false} opacity=".2"/><XAxis type="number" tick={{fontSize:12}} unit=" kg"/><YAxis type="category" dataKey="department" width={110} tick={{fontSize:12}}/><Tooltip formatter={value=>[`${Number(value).toLocaleString()} kg CO₂e`,'Emissions']} cursor={{fill:'#f1f5f9'}}/><Bar dataKey="emissions" fill="#059669" radius={[0,6,6,0]} barSize={24}/></BarChart></ResponsiveContainer>:<Empty label="department emission data"/>}</div>
    </section>
    <section className={`${card} p-0`}>
      <div className="border-b border-slate-200 px-5 py-4"><h2 className="text-base font-semibold">Department performance</h2><p className="mt-1 text-sm text-slate-500">Ranked from the lowest to highest carbon footprint.</p></div>
      <div className="overflow-x-auto">
        <table className="w-full min-w-[720px] text-sm">
          <thead className="bg-slate-50 text-left text-xs font-semibold uppercase tracking-wide text-slate-500"><tr><th className="px-5 py-3">Rank</th><th className="px-5 py-3">Department</th><th className="px-5 py-3">Employees</th><th className="px-5 py-3">Emissions</th><th className="px-5 py-3">Share of total</th><th className="px-5 py-3">Performance</th></tr></thead>
          <tbody>{sorted.map((row,index)=>{const emission=Number(row.emissions||0);const share=total?emission*100/total:0;const relative=highest?emission*100/highest:0;return <tr key={row.department} className="border-t border-slate-100 hover:bg-slate-50/70"><td className="px-5 py-4"><span className={`grid h-8 w-8 place-items-center rounded-full text-xs font-semibold ${index===0?'bg-emerald-100 text-emerald-800':'bg-slate-100 text-slate-600'}`}>{index+1}</span></td><td className="px-5 py-4 font-semibold text-slate-900">{row.department}</td><td className="px-5 py-4 text-slate-600">{Number(row.employees||0).toLocaleString()}</td><td className="px-5 py-4 font-medium">{emission.toLocaleString()} kg CO₂e</td><td className="px-5 py-4"><div className="flex items-center gap-3"><div className="h-2 w-28 overflow-hidden rounded-full bg-slate-100"><div className="h-full rounded-full bg-emerald-500" style={{width:`${relative}%`}}/></div><span className="text-xs text-slate-500">{share.toFixed(1)}%</span></div></td><td className="px-5 py-4"><span className={`rounded-full px-2.5 py-1 text-xs font-semibold ${index===0?'bg-emerald-50 text-emerald-700':index===sorted.length-1&&sorted.length>1?'bg-amber-50 text-amber-700':'bg-blue-50 text-blue-700'}`}>{index===0?'Leading':index===sorted.length-1&&sorted.length>1?'Needs attention':'On track'}</span></td></tr>})}</tbody>
        </table>
        {!sorted.length&&<Empty label="department data"/>}
      </div>
    </section>
  </div>;
}
function Employees({rows=[],reload}){
  ...
  return <section className={card}><div className="mb-4 flex flex-col gap-3 md:flex-row"><div className="relative flex-1"><Search className="absolute left-3 top-3 h-4 w-4 text-slate-400"/><input className={`${input} pl-9`} value={search} onChange={e=>{setSearch(e.target.value);setPage(1)}} placeholder="Search employee name or email"/></div><select className={input} value={department} onChange={e=>setDepartment(e.target.value)}><option value="">All departments</option>{departments.map(d=><option key={d}>{d}</option>)}</select><select className={input} value={sort} onChange={e=>setSort(e.target.value)}><option value="name">Sort by name</option><option value="score">Sort by score</option><option value="emission">Sort by emission</option></select></div>
    <div className="overflow-x-auto"><table className="w-full min-w-[900px] text-sm"><thead className="bg-slate-50 text-left text-xs uppercase text-slate-500 dark:bg-white/5"><tr>{['Employee Name','Department','Carbon Score','Monthly Emission','Activities','Goal Progress','Status'].map(h=><th key={h} className="px-4 py-3">{h}</th>)}</tr></thead><tbody>{visible.map(r=><tr key={r.id} onClick={()=>setSelected(r)} className="cursor-pointer border-t border-slate-100 hover:bg-emerald-50/50 dark:border-white/5 dark:hover:bg-white/5"><td className="px-4 py-3 font-bold">{r.name}<span className="block text-xs font-normal text-slate-500">{r.email}</span></td><td className="px-4">{r.department}</td><td className="px-4 font-bold text-emerald-600">{r.carbonScore}</td><td className="px-4">{r.monthlyEmission} kg</td><td className="px-4">{r.activities}</td><td className="px-4"><div className="h-2 w-28 rounded bg-slate-200"><div className="h-full rounded bg-emerald-500" style={{width:`${Math.min(100,r.goalProgress)}%`}}/></div><span className="text-xs">{r.goalProgress}%</span></td><td className="px-4"><span className="rounded-full bg-emerald-100 px-2 py-1 text-xs font-bold text-emerald-700">{r.status}</span></td></tr>)}</tbody></table>{!visible.length&&<Empty label="employees"/>}</div>
    <div className="mt-4 flex items-center justify-between text-sm"><span>{filtered.length} employees</span><div className="flex gap-2"><button disabled={page===1} onClick={()=>setPage(p=>p-1)} className="rounded-lg border px-3 py-1 disabled:opacity-40">Previous</button><span className="px-2 py-1">{page}/{pages}</span><button disabled={page===pages} onClick={()=>setPage(p=>p+1)} className="rounded-lg border px-3 py-1 disabled:opacity-40">Next</button></div></div>
    {selected&&<div className="fixed inset-0 z-50 bg-slate-950/50" onClick={()=>setSelected(null)}><aside className="absolute right-0 h-full w-full max-w-md bg-white p-7 shadow-2xl dark:bg-slate-900" onClick={e=>e.stopPropagation()}><button className="float-right" onClick={()=>setSelected(null)}><X/></button><div className="mt-12 grid h-20 w-20 place-items-center rounded-2xl bg-emerald-100 text-3xl font-black text-emerald-700">{selected.name?.[0]}</div><h2 className="mt-5 text-2xl font-black">{selected.name}</h2><p className="text-slate-500">{selected.email}</p><dl className="mt-8 grid grid-cols-2 gap-4">{Object.entries({Department:selected.department,Phone:selected.phone||'Not provided','Carbon score':selected.carbonScore,'Monthly emission':`${selected.monthlyEmission} kg`,Activities:selected.activities,'Goal progress':`${selected.goalProgress}%`,Status:selected.status}).map(([k,v])=><div key={k} className="rounded-xl bg-slate-50 p-3 dark:bg-white/5"><dt className="text-xs text-slate-500">{k}</dt><dd className="mt-1 font-bold">{v}</dd></div>)}</dl><button onClick={()=>{setEditing({id:selected.id,fullName:selected.name,email:selected.email,department:selected.department==='Unassigned'?'':selected.department,phone:selected.phone||'',status:selected.status||'ACTIVE'});setSelected(null)}} className="mt-6 w-full rounded-lg bg-emerald-600 px-4 py-3 text-sm font-semibold text-white">Edit Employee Details</button></aside></div>}
    {editing&&<div className="fixed inset-0 z-50 grid place-items-center bg-slate-950/60 p-4"><form onSubmit={async e=>{e.preventDefault();setSaving(true);try{await updateOrganisationEmployee(editing.id,editing);await reload(true);toast.success('Employee details updated');setEditing(null)}catch(err){toast.error(err.response?.data?.error||err.response?.data?.message||'Unable to update employee')}finally{setSaving(false)}}} className={`${card} w-full max-w-xl`}><div className="flex items-center justify-between"><div><h2 className="text-xl font-bold">Edit Employee</h2><p className="mt-1 text-sm text-slate-500">Update organisation employee information.</p></div><button type="button" onClick={()=>setEditing(null)} aria-label="Close edit employee"><X/></button></div><div className="mt-5 grid gap-4 sm:grid-cols-2"><label><span className="mb-1.5 block text-sm font-medium">Full name</span><input required className={input} value={editing.fullName} onChange={e=>setEditing({...editing,fullName:e.target.value})}/></label><label><span className="mb-1.5 block text-sm font-medium">Work email</span><input required type="email" className={input} value={editing.email} onChange={e=>setEditing({...editing,email:e.target.value})}/></label><label><span className="mb-1.5 block text-sm font-medium">Department</span><input className={input} placeholder="e.g. Operations" value={editing.department} onChange={e=>setEditing({...editing,department:e.target.value})}/></label><label><span className="mb-1.5 block text-sm font-medium">Phone</span><input className={input} placeholder="e.g. +91 98765 43210" value={editing.phone} onChange={e=>setEditing({...editing,phone:e.target.value})}/></label><label className="sm:col-span-2"><span className="mb-1.5 block text-sm font-medium">Participation status</span><select className={input} value={editing.status} onChange={e=>setEditing({...editing,status:e.target.value})}><option value="ACTIVE">Active</option><option value="INACTIVE">Inactive</option></select></label></div><div className="mt-6 flex justify-end gap-3"><button type="button" onClick={()=>setEditing(null)} className="h-11 rounded-lg border border-slate-300 px-4 text-sm font-semibold dark:border-slate-700">Cancel</button><button disabled={saving} className="h-11 rounded-lg bg-emerald-600 px-5 text-sm font-semibold text-white disabled:opacity-50">{saving?'Saving…':'Save Changes'}</button></div></form></div>}
  </section>;
}
...
function getReportTable(data,type,period){
  ...
  if(type==='Monthly Sustainability Summary'){
    const latest=monthly.at(-1);
    return {headers:['Measure','Current value','Context'],rows:[
      ['Tracked emissions',`${latest?.emissions||0} kg CO₂e`,latest?.month||period],
      ['Emission reduction',`${data.kpis?.monthlyReduction||0}%`,'Compared with previous month'],
      ['Participation rate',`${data.kpis?.participationRate||0}%`,`${data.kpis?.totalEmployees||0} employees`],
      ['Active goals',data.kpis?.activeGoals||0,'Organisation sustainability goals'],
      ['Sustainability score',data.kpis?.averageCarbonScore||0,'Current organisation score'],
    ]};
  }
  if(type==='Quarterly CSR Report'){
    const rows=monthly.slice(-3);const total=rows.reduce((sum,row)=>sum+Number(row.emissions||0),0);
    return {headers:['Quarter month','Emissions','Share of quarter'],rows:rows.map(row=>[row.month,`${row.emissions} kg CO₂e`,`${total?Number(row.emissions||0)*100/total:0}`.slice(0,5)+'%'])};
  }
  if(type==='Annual Emission Report'){
    const rows=monthly.slice(-12);const total=rows.reduce((sum,row)=>sum+Number(row.emissions||0),0);
    return {headers:['Month','Emissions','Share of annual total'],rows:rows.map(row=>[row.month,`${row.emissions} kg CO₂e`,`${(total?Number(row.emissions||0)*100/total:0).toFixed(1)}%`])};
  }
  if(type==='Department Performance Report')return {headers:['Department','Employees','Emissions'],rows:(data.departmentComparison||[]).map(row=>[row.department,row.employees,`${row.emissions} kg CO₂e`])};
  return {headers:['Employee','Department','Participation details'],rows:(data.employees||[]).map(row=>[row.name,row.department,`${row.status} · ${row.activities} activities · Score ${row.carbonScore}`])};
}

function ReportDocumentPreview({data,type,period}){
  const metrics=[['Total emissions',`${Number(data.kpis?.totalEmission||0).toLocaleString()} kg CO₂e`],['Participation',`${data.kpis?.participationRate||0}%`],['Employees',data.kpis?.totalEmployees||0],['Sustainability score',data.kpis?.averageCarbonScore||0]];
  const {headers,rows}=getReportTable(data,type,period);
  return <article className="mt-5 overflow-hidden rounded-xl border border-slate-200 bg-white shadow-sm dark:border-slate-700 dark:bg-slate-900">
    <header className="flex flex-col justify-between gap-5 bg-[#0b271e] px-6 py-6 text-white sm:flex-row">
      <div><div className="flex items-center gap-2"><Leaf className="h-6 w-6 text-emerald-400"/><strong className="text-xl">CarbonTrack</strong></div><p className="mt-1 text-sm text-emerald-100">Organisation Sustainability Report</p></div>
      <div className="text-left sm:text-right"><p className="font-semibold">{type}</p><p className="mt-1 text-sm text-emerald-100">{period}</p></div>
    </header>
    <div className="p-6">
      <div className="flex flex-col justify-between gap-3 border-b border-slate-200 pb-5 dark:border-slate-700 sm:flex-row sm:items-end"><div><h3 className="text-2xl font-bold text-slate-950 dark:text-white">{data.organisation?.name}</h3><p className="mt-1 text-sm text-slate-500">Generated {new Date().toLocaleString()}</p></div><span className="w-fit rounded-full bg-emerald-50 px-3 py-1 text-xs font-semibold text-emerald-700 dark:bg-emerald-950/50 dark:text-emerald-300">Verified live organisation data</span></div>
      <div className="my-6 grid gap-3 sm:grid-cols-2 xl:grid-cols-4">{metrics.map(([label,value])=><div key={label} className="rounded-lg border border-emerald-100 bg-emerald-50/60 p-4 dark:border-emerald-900 dark:bg-emerald-950/30"><p className="text-xs font-medium text-slate-500">{label}</p><p className="mt-2 text-lg font-bold text-emerald-800 dark:text-emerald-300">{value}</p></div>)}</div>
      <h4 className="mb-3 text-base font-semibold">Report details</h4>
      <div className="overflow-x-auto rounded-lg border border-slate-200 dark:border-slate-700"><table className="w-full min-w-[560px] text-sm"><thead className="bg-slate-50 text-left text-xs uppercase tracking-wide text-slate-500 dark:bg-slate-800"><tr>{headers.map(header=><th key={header} className="px-4 py-3">{header}</th>)}</tr></thead><tbody>{rows.map((row,index)=><tr key={`${row[0]}-${index}`} className="border-t border-slate-100 even:bg-slate-50/60 dark:border-slate-800 dark:even:bg-slate-800/40">{row.map((value,column)=><td key={column} className={`px-4 py-3 ${column===0?'font-semibold':''}`}>{value}</td>)}</tr>)}</tbody></table>{!rows.length&&<Empty label="verified report records"/>}</div>
      <footer className="mt-6 flex flex-col justify-between gap-2 border-t border-slate-200 pt-4 text-xs text-slate-400 dark:border-slate-700 sm:flex-row"><span>Generated from CarbonTrack organisation data</span><span>CarbonTrack · Sustainability intelligence</span></footer>
    </div>
  </article>;
}

function ReportCentre({data}){
  const types=['Monthly Sustainability Summary','Quarterly CSR Report','Annual Emission Report','Department Performance Report','Employee Participation Report'];
  const [type,setType]=useState(types[0]);const [period,setPeriod]=useState('This month');const [preview,setPreview]=useState('');const [busy,setBusy]=useState(false);
  const filename=()=>type.toLowerCase().replaceAll(' ','-');
  const summary=useMemo(()=>`CARBONTRACK CSR REPORT
Organisation: ${data.organisation?.name||'Organisation'}
Generated: ${new Date().toLocaleDateString()}
Employees: ${data.kpis?.totalEmployees||0}
Tracked emissions: ${data.kpis?.totalEmission||0} kg CO2e
Participation: ${data.kpis?.participationRate||0}%
Sustainability score: ${data.kpis?.averageCarbonScore||0}/100`,[data]);
  const compose=useCallback(server=>{
    const count={'This month':1,'Last month':1,'This quarter':3,'This year':12}[period];
    const lines=[server.trim(),'',`Report type: ${type}`,`Reporting period: ${period}`,''];
    if(type==='Department Performance Report')lines.push('DEPARTMENT PERFORMANCE','Department | Employees | Emissions (kg CO2e)',...(data.departmentComparison||[]).map(row=>`${row.department} | ${row.employees} | ${row.emissions}`));
    else if(type==='Employee Participation Report')lines.push('EMPLOYEE PARTICIPATION','Employee | Department | Status | Activities | Carbon score',...(data.employees||[]).map(row=>`${row.name} | ${row.department} | ${row.status} | ${row.activities} | ${row.carbonScore}`));
    else lines.push('EMISSION TREND','Month | Emissions (kg CO2e)',...(data.monthlyEmissions||[]).slice(-count).map(row=>`${row.month} | ${row.emissions}`),'','KEY PERFORMANCE INDICATORS',...Object.entries(data.kpis||{}).map(([key,value])=>`${key}: ${value}`));
    return lines.join('\n');
  },[data,period,type]);
  useEffect(()=>{setPreview(compose(summary))},[compose,summary]);
  const prepare=async()=>{setBusy(true);const value=compose(summary);setPreview(value);setBusy(false);return value};
  const save=(blob,extension)=>{const url=URL.createObjectURL(blob);const anchor=document.createElement('a');anchor.href=url;anchor.download=`${filename()}.${extension}`;document.body.appendChild(anchor);anchor.click();anchor.remove();setTimeout(()=>URL.revokeObjectURL(url),1000)};
  const pdf=async()=>{
    const text=preview||await prepare();if(!text)return;
    const {jsPDF}=await import('jspdf');const documentPdf=new jsPDF();const pageWidth=documentPdf.internal.pageSize.getWidth();
    const header=()=>{documentPdf.setFillColor(11,39,30);documentPdf.rect(0,0,pageWidth,35,'F');documentPdf.setTextColor(255,255,255);documentPdf.setFont('helvetica','bold');documentPdf.setFontSize(20);documentPdf.text('CarbonTrack',18,15);documentPdf.setFontSize(11);documentPdf.setFont('helvetica','normal');documentPdf.text('Organisation Sustainability Report',18,24);documentPdf.setFontSize(9);documentPdf.text(type,pageWidth-18,15,{align:'right'});documentPdf.text(period,pageWidth-18,23,{align:'right'});documentPdf.setTextColor(15,23,42)};
    header();let y=46;
    documentPdf.setFont('helvetica','bold');documentPdf.setFontSize(16);documentPdf.text(data.organisation?.name||'Organisation',18,y);y+=7;documentPdf.setFont('helvetica','normal');documentPdf.setFontSize(9);documentPdf.setTextColor(100,116,139);documentPdf.text(`Generated ${new Date().toLocaleString()}  |  Verified CarbonTrack organisation data`,18,y);documentPdf.setTextColor(15,23,42);y+=12;
    const metrics=[['Total emissions',`${Number(data.kpis?.totalEmission||0).toLocaleString()} kg CO2e`],['Participation',`${data.kpis?.participationRate||0}%`],['Employees',String(data.kpis?.totalEmployees||0)],['Sustainability score',String(data.kpis?.averageCarbonScore||0)]];
    metrics.forEach(([label,value],index)=>{const x=18+index*44;documentPdf.setFillColor(240,253,244);documentPdf.roundedRect(x,y,40,23,3,3,'F');documentPdf.setTextColor(71,85,105);documentPdf.setFontSize(8);documentPdf.text(label,x+4,y+7);documentPdf.setTextColor(6,95,70);documentPdf.setFont('helvetica','bold');documentPdf.setFontSize(11);documentPdf.text(value,x+4,y+16,{maxWidth:34});documentPdf.setFont('helvetica','normal')});y+=34;
    documentPdf.setFont('helvetica','bold');documentPdf.setFontSize(13);documentPdf.setTextColor(15,23,42);documentPdf.text('Report details',18,y);y+=8;
    let rows=getReportTable(data,type,period).rows;
    if(!rows.length)rows=[['No records','No verified records are available for the selected report.','—']];
    rows.forEach((row,index)=>{if(y>268){documentPdf.addPage();header();y=46}if(index%2===0){documentPdf.setFillColor(248,250,252);documentPdf.rect(18,y-5,174,12,'F')}documentPdf.setFontSize(9);documentPdf.setTextColor(30,41,59);documentPdf.setFont('helvetica','bold');documentPdf.text(String(row[0]),22,y);documentPdf.setFont('helvetica','normal');documentPdf.setTextColor(71,85,105);documentPdf.text(String(row[1]),82,y,{maxWidth:55});documentPdf.setTextColor(15,118,110);documentPdf.text(String(row[2]),188,y,{align:'right',maxWidth:48});y+=12});
    const pages=documentPdf.internal.getNumberOfPages();for(let page=1;page<=pages;page++){documentPdf.setPage(page);documentPdf.setDrawColor(226,232,240);documentPdf.line(18,285,192,285);documentPdf.setFontSize(8);documentPdf.setTextColor(100,116,139);documentPdf.text('Generated from live CarbonTrack organisation data',18,291);documentPdf.text(`Page ${page} of ${pages}`,192,291,{align:'right'})}
    documentPdf.save(`${filename()}.pdf`);toast.success('Professional PDF report downloaded');
  };
  const csv=async()=>{if(!preview&&!await prepare())return;const quote=value=>`"${String(value??'').replaceAll('"','""')}"`;const table=getReportTable(data,type,period);const rows=[['Report Type',type],['Period',period],['Organisation',data.organisation?.name||''],[],table.headers,...table.rows];save(new Blob(['\uFEFF'+rows.map(row=>row.map(quote).join(',')).join('\r\n')],{type:'text/csv;charset=utf-8'}),'csv');toast.success('CSV report downloaded')};
  return <div className="space-y-5"><section className={card}><div className="grid gap-3 lg:grid-cols-[1fr_200px_auto_auto_auto]"><label><span className="mb-1.5 block text-xs font-medium text-slate-600">Report type</span><select className={input} value={type} onChange={event=>{setType(event.target.value);setPreview('')}}>{types.map(value=><option key={value}>{value}</option>)}</select></label><label><span className="mb-1.5 block text-xs font-medium text-slate-600">Period</span><select className={input} value={period} onChange={event=>{setPeriod(event.target.value);setPreview('')}}><option>This month</option><option>Last month</option><option>This quarter</option><option>This year</option></select></label><button disabled={busy} onClick={prepare} className="mt-auto flex h-11 items-center justify-center gap-2 rounded-lg border border-slate-300 px-4 text-sm font-semibold disabled:opacity-50"><Eye className="h-4 w-4"/>{busy?'Loading…':'Refresh'}</button><button disabled={busy} onClick={pdf} className="mt-auto flex h-11 items-center justify-center gap-2 rounded-lg bg-emerald-600 px-4 text-sm font-semibold text-white disabled:opacity-50"><Download className="h-4 w-4"/>PDF</button><button disabled={busy} onClick={csv} className="mt-auto flex h-11 items-center justify-center gap-2 rounded-lg border border-emerald-600 px-4 text-sm font-semibold text-emerald-700 disabled:opacity-50 dark:text-emerald-300"><FileSpreadsheet className="h-4 w-4"/>CSV</button></div></section><section className={card}><div className="flex items-start justify-between gap-4"><div><h2 className="text-lg font-semibold">Report preview</h2><p className="mt-1 text-sm text-slate-500">This preview matches the structure of the downloaded report.</p></div>{preview&&<EcoLottie animationData={successAnimation} loop={false} className="h-16 w-16 shrink-0" fallback={<EcoStaticFallback type="success"/>} reducedMotionFallback={<EcoStaticFallback type="success"/>}/>}</div>{preview?<ReportDocumentPreview data={data} type={type} period={period}/>:<div className="mt-5"><Empty label="generated report preview"/></div>}</section></div>;
}
...
export default function OrganisationPortalPage(){
  ...
  if(loading&&!['dashboard','analytics','monthly-trends','departments','employees','top-contributors','lowest-footprint','activity-logs','goals','profile'].includes(section))return <OrganisationEcoLoader/>;if(error&&!['dashboard','analytics','monthly-trends','departments','employees','top-contributors','lowest-footprint','activity-logs','goals','profile'].includes(section))return <div className={`${card} border-red-200 text-red-700`}>{error}<button className="ml-3 underline" onClick={load}>Retry</button></div>;
