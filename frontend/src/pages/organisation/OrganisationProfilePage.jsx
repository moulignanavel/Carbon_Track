...
export default function OrganisationProfilePage({ data, loading, error, onRetry, onReload = onRetry }) {
      ...
      <section className={`${card} text-rose-700`}>
        <div className="flex items-center gap-3">
          ...
          <div className="flex-1">
            <h1 className="font-semibold">Organisation profile could not be loaded</h1>
  ...
  return (
    <form onSubmit={submit} className="space-y-4">
      <header>
        <p className="text-[10px] font-extrabold uppercase tracking-[.16em] text-emerald-700 dark:text-emerald-300">Organisation settings</p>
        <h1 className="mt-1 text-xl font-black tracking-tight text-slate-950 dark:text-white sm:text-2xl">Organisation Profile</h1>
        <p className="mt-0.5 text-xs text-slate-500">Maintain organisation identity, address, reporting preferences and branding.</p>
      </header>
      <div className="grid gap-3.5 lg:grid-cols-2">
        <Section title="Organisation Information" icon={Building2}>
          <Field label="Organisation Name" name="name" value={form.name} onChange={change} error={errors.name} required />
          <Field label="Organisation Code" name="code" value={form.code} onChange={change} />
          <Field label="Industry" name="industry" value={form.industry} onChange={change} />
          <Field label="Type" name="organisationType" value={form.organisationType} onChange={change} />
          <Field label="Official Email" name="email" type="email" value={form.email} onChange={change} error={errors.email} />
          <Field label="Phone" name="phone" type="tel" value={form.phone} onChange={change} />
          <Field label="Website" name="website" type="url" value={form.website} onChange={change} error={errors.website} wide />
        </Section>
        <Section title="Address" icon={Leaf}>
          <Field label="Address Line" name="address" value={form.address} onChange={change} wide />
          ...
          <Field label="Postal Code" name="postalCode" value={form.postalCode} onChange={change} />
        </Section>
        <Section title="Sustainability Settings" icon={Leaf}>
          <Field label="Annual Carbon Target" name="carbonTarget" type="number" value={form.carbonTarget} onChange={change} error={errors.carbonTarget} />
          <Field label="Reporting Year" name="reportingYear" type="number" value={form.reportingYear} onChange={change} error={errors.reportingYear} />
          <label>
            <span className="mb-1 block text-xs font-semibold text-slate-600 dark:text-slate-300">Preferred Unit</span>
          ...
          <label>
            <span className="mb-1 block text-xs font-semibold text-slate-600 dark:text-slate-300">Reporting Frequency</span>
        ...
        <Section title="Branding" icon={Image}>
          <label className="sm:col-span-2">
            <span className="mb-1 block text-xs font-semibold text-slate-600 dark:text-slate-300">Organisation Logo</span>
            ...
            <span className="mt-1 block text-[10px] text-slate-400">PNG, JPEG, WebP or SVG · maximum 750 KB</span>
          </label>
          <div className="sm:col-span-2">
            <p className="mb-1.5 text-xs font-semibold text-slate-600 dark:text-slate-300">Logo Preview</p>
            <div className="grid min-h-32 place-items-center rounded-lg border border-dashed border-slate-300 bg-slate-50 p-4 dark:border-slate-700 dark:bg-slate-800">
              {form.logoUrl ? (
                ...
                <div className="text-center text-slate-400">
                  ...
                  <p className="mt-1 text-xs">No logo selected</p>
                </div>
              )}
            </div>
          </div>
      ...
      {success && (
        <motion.div initial={reduceMotion ? false : { opacity: 0, scale: 0.96 }} animate={{ opacity: 1, scale: 1 }} className="fixed bottom-24 right-5 z-50 rounded-xl bg-white p-3.5 text-center shadow-2xl dark:bg-slate-900" aria-live="polite">
          ...
          <p className="text-xs font-bold text-emerald-700 dark:text-emerald-300">Profile saved</p>
        </motion.div>
      )}
    </form>
  );
