    ...
    >
      ...
      <div className="flex-1 min-w-0">
        <p className={`text-sm font-medium truncate ${isCurrentUser ? 'text-green-700 dark:text-green-400' : 'text-slate-800 dark:text-slate-200'}`}>
          ...
          {isCurrentUser && (
            <Badge variant="green" size="xs" className="ml-2">You</Badge>
          )}
        </p>
        <p className="text-xs text-slate-400 dark:text-slate-500 tabular-nums">
          {formatEmission(monthly)}
        </p>
      </div>
...
export default function Leaderboard({ entries, isLoading }) {
  if (isLoading) {
    return (
      <Card>
        <Card.Header title="Leaderboard" icon={Trophy} />
  ...
  return (
    <Card>
      ...
      {(() => {
        ...
        if (!you) return null;
        const ahead = entries.filter((e) => !e.isCurrentUser && e.monthly < you.monthly).length;
        const behind = entries.filter((e) => !e.isCurrentUser && e.monthly > you.monthly).length;
        return (
          <div className="mt-4 rounded-xl bg-green-50 dark:bg-green-900/20 border border-green-200 dark:border-green-900/40 px-4 py-3">
            <p className="text-xs text-green-700 dark:text-green-400 font-medium">
              You&apos;re rank <span className="font-bold">{you.rank}</span> — reduce by{' '}
              <span className="font-bold">{formatEmission(you.monthly - (entries.find((e) => e.rank === you.rank - 1)?.monthly ?? 0))}</span>{' '}
              to climb one spot.
            </p>
          </div>
        );
